...
<div class="content-wrapper">
	...
	<div class="content clearfix">
		<form class="news" action="updateUser" method="post">
			<s:hidden value="%{#request.user.id}" name="user.id"/>
			<s:textfield id="newscontent"  value="%{#request.user.shortdec}" />
			<b>姓名</b>
			<br>
			<s:textfield type="text" name="user.name"  value="%{#request.user.name}"/>
			<br>
			<b>用户名</b>
			<br>
			<s:textfield type="text" name="user.username"  value="%{#request.user.username}"/>
			<br>
			<b>密码</b>
			<br>
			<s:textfield type="text" name="user.password"  value="%{#request.user.password}"/>
			<br>
			<b>成员类型</b>
			<br>
			<s:select name="user.type" list="#{0:'超级管理员',1:'管理员',2:'普通成员'}" theme="simple" listKey="key" listValue="value"  value="#request.user.type" style="width:80px;"></s:select>
			<br>
			<b>加入时间</b>
			<br>
